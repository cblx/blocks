...
{
    private static T GetOrThrow<T>(this in ImmutableArray<TypedConstant> attributeParameters, int index,
        string propertyName)
        => (T)(attributeParameters[index].Value ?? throw new ConfigurationException($"{propertyName} cannot be null."));

    public static string GetOrThrow(this in ImmutableArray<TypedConstant> attributeParameters, int index,
        string propertyName)
        => attributeParameters.GetOrThrow<string>(index, propertyName);

    private static T GetOrThrow<T>(this in ImmutableArray<KeyValuePair<string, TypedConstant>> attributeParameters,
        string propertyName)
    {
    ...
    public static string GetOrThrow(
        this in ImmutableArray<KeyValuePair<string, TypedConstant>> attributeParameters,
        string propertyName)
    {
        return attributeParameters.GetOrThrow<string>(propertyName);
    }

    public static T? GetOrDefault<T>(this in ImmutableArray<KeyValuePair<string, TypedConstant>> attributeParameters,
        string propertyName)
    {
